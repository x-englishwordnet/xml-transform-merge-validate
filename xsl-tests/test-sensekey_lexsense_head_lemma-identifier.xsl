...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:dc="http://purl.org/dc/elements/1.1/">
	...
	<xsl:template match="/">
		...
		./@dc:identifier != '' and 
		substring-before(substring-after(substring-after(substring-after(substring-after(./@dc:identifier,'%'),':'),':'),':'),':') != 
		substring-before(substring-after(substring-after(substring-after(substring-after(./@sensekey,'%'),':'),':'),':'),':')
		]" />
		<xsl:for-each select="$diff_scope">
			<xsl:call-template name="sense-node-diff">
				<xsl:with-param name="var1" select="substring-before(substring-after(substring-after(substring-after(substring-after(./@dc:identifier,'%'),':'),':'),':'),':')" />
				<xsl:with-param name="var2" select="substring-before(substring-after(substring-after(substring-after(substring-after(./@sensekey,'%'),':'),':'),':'),':')" />
			</xsl:call-template>
		</xsl:for-each>
